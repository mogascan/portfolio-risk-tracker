"""
Market data service for CoinGecko API integration
"""
import aiohttp
import asyncio
import json
...
class MarketDataService:
    ...
    async def get_prices(self, symbols: Optional[List[str]] = None, limit: int = 100) -> List[Dict[str, Any]]:
        ...
        Args:
            ...
            limit: Number of cryptocurrencies to fetch if symbols not provided (max 500)
        ...
        try:
            ...
            if symbols:
                ...
                coins = await self.coingecko_service.get_top_coins(limit=500)  # Get more coins to improve match chances
            ...
            # Don't silently fall back to empty list, provide error info
            error_message = str(e)
            if "Rate limit" in error_message:
                raise Exception("CoinGecko API rate limit exceeded. Please try again in a few minutes.")
            elif "403" in error_message:
                raise Exception("Access to CoinGecko API is currently restricted. This may be due to IP-based rate limiting.")
            elif "Connection" in error_message:
                raise Exception("Could not connect to CoinGecko API. Please check your internet connection and try again.")
            else:
                raise Exception(f"Error fetching cryptocurrency prices: {error_message}")
